...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if not connected:
            ...
            logger.info(f"✅ Connected to Groq with model: {chatbot.model_name}")
        ...
        logger.info("🔄 API starting in degraded mode. Check GROQ_API_KEY configuration.")
...
async def health_check():
    ...
    if not chatbot:
        # Return helpful information even when Groq isn't connected
...
async def chat_endpoint(
    ...
    if not chatbot:
        raise HTTPException(
            ...
            detail="Chatbot service unavailable. Please configure GROQ_API_KEY environment variable and ensure Groq is accessible."
        )
    
    # Check Groq connection before processing
    connected, message = chatbot.check_connection()
    if not connected:
    ...
    try:
        ...
        logger.warning(f"Groq request timed out for IP: {client_request.client.host}")
